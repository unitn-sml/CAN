...
import semantic_losses


class GraphGANOptimizer(object):

    def __init__(self, model, learning_rate=1e-3, feature_matching=True, sl_use_sigmoid=False):
        self.la = tf.placeholder_with_default(1., shape=())
        self.la_SL = tf.placeholder_with_default(1., shape=())
        ...
        with tf.name_scope('losses'):
            ...
            self.loss_SL, self.SL_log_dict = semantic_losses.SemanticLoss(sl_use_sigmoid).forward(model.nodes_hat,
                                                                                                  model.edges_hat,
                                                                                                  model.embeddings)
        ...
        alpha = tf.abs(tf.stop_gradient(self.loss_G / (self.loss_RL + 1e-10)))
        alpha_SL = tf.abs(tf.stop_gradient(self.loss_G / (self.loss_SL + 1e-10)))
        ...
        with tf.name_scope('train_step'):
            ...
            min_lambda = tf.cond(self.la > self.la_SL, lambda: self.la_SL, lambda: self.la)
            self.train_step_G = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(
                # adversarial
                loss=tf.cond(tf.greater(self.la, 0), lambda: min_lambda * self.loss_G, lambda: 0.)
                     # RL
                     + tf.cond(tf.less(self.la, 1), lambda: (1 - self.la) * alpha * self.loss_RL, lambda: 0.)
                     # SL
                     + tf.cond(tf.less(self.la_SL, 1), lambda: (1 - self.la_SL) * alpha_SL * self.loss_SL, lambda: 0.),
