...
class ConstraintsToDimacs:
    ...
    Refer to the documentation of expression_to_dimacs for more information.
    Usage:
        ConstraintsToDimacs.expression_to_dimacs(input file, output file)
    """

    @classmethod
    def expression_to_dimacs(cls, constraint_file, output_file, nprocesses=1, simplify=False, dnf = False):
        """
        ...
        shape, constraints = ConstraintsToDimacs._read_data(constraint_file)
        ...
        if nprocesses == 1:
            constraints_expression = ConstraintsToDimacs._parse_constraints_monoprocess(constraints)
        else:
            constraints_expression = ConstraintsToDimacs._parse_constraints_multiprocess(constraints, nprocesses)

        # check variables for correctness (shape wise and name/format)
        if not dnf:
            for var in constraints_expression.atoms():
                ConstraintsToDimacs._assert_is_valid_variable(str(var), shape, stride, total_vars)
        ...
        ConstraintsToDimacs._to_dimacs(shape, stride, total_vars, constraints_expression, constraint_file, output_file, dnf)
    ...
    def _parse_constraints_multiprocess(constraints, nprocesses):
        ...
        with Pool(processes=nprocesses) as pool:
            multiple_results = [pool.apply_async(ConstraintsToDimacs._parse_constraints_monoprocess, [chunk]) for chunk in
    ...
    def _assert_is_valid_variable(var, shape, stride, total_vars):
        ...
        """
        assert ConstraintsToDimacs._get_index(np.array(items), stride) < total_vars
    ...
    @staticmethod
    def _to_dimacs(shape, stride, total_vars, cnf, input_file, output_file, dnf):
        """
        ...
        :param output_file: Refer to expression_to_dimacs.
        """
        ...
        for var in variables:
            ...
            items = list(map(lambda x: int(x), regex.findall("[0-9]+", var)))
            index = ConstraintsToDimacs._get_index(np.array(items), stride)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--simplify", action="store_true", help="Whether CNF should be simplified")
    parser.add_argument("-d", "--dnf", action="store_true", default=False, help="Input is dnf")

    args = parser.parse_args()
    ConstraintsToDimacs.expression_to_dimacs(args.input, args.output, args.nprocesses, args.simplify, args.dnf)
