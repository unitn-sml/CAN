...
class Trainer:
    ...
    def train(self, batch_dim, epochs, steps,
        ...
        import os
        if os.path.exists(directory + "/checkpoint"):
            self.load(directory)
        ...
        if _eval_step is None:
            def _eval_step(epoch, epochs, min_epochs, model, optimizer, batch_dim, eval_batch, start_time,
                ...
                fname = directory + "/evaluation.csv"
                ...
                with open(fname, "a") as file:
                    values = ",".join([str(output[k]) for k in sorted(output)]) + "\n"
                    file.write(values)
    ...
    def test(self, batch_dim,
              test_fetch_dict, test_feed_dict,
             _test_step=None,
             _eval_update=None, _test_update=None,
             directory=None):
        import os
        if os.path.exists(directory + "/checkpoint"):
            self.load(directory)
        ...
        if _test_step is None:
            def _test_step(model, optimizer, batch_dim,
                           _test_update):
                self.load(directory)
                feed_dict, z_discrete = test_feed_dict(model, optimizer, batch_dim)
                output = self.session.run(test_fetch_dict(model, optimizer),
                                              feed_dict=feed_dict)
                ...
                output = _test_update(model, None, batch_dim, None)

                return output, z_discrete

        return _test_step(self.model, self.optimizer, batch_dim, _test_update)
