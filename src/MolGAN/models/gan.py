...
class GraphGANModel(object):
    ...
    def __init__(self, vertexes, edges, nodes, embedding_dim, decoder_units, discriminator_units,
                 ...
                 batch_discriminator=True, discrete_z=0):
    ...
    def sample_z(self, batch_dim):
        a = np.random.normal(0, 1, size=(batch_dim, self.embedding_dim - self.discrete_z))
        if self.discrete_z > 0:
            b = np.random.uniform(0, 1, size=(batch_dim, self.discrete_z))
            a = np.concatenate([a, b], axis=1)
        return a

    def sample_z_test(self, batch_dim):
        a = np.random.normal(0, 1, size=(batch_dim, self.embedding_dim - self.discrete_z))
        discr_z = []
        if self.discrete_z > 0:
            discr_z = np.eye(self.discrete_z)[np.random.choice(self.discrete_z, batch_dim)]
            a = np.concatenate([a, discr_z], axis=1)
        return a, discr_z
