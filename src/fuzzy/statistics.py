...
import fuzzy.losses as losses
from fuzzy.util import _get_constraints_names


class FuzzyLogicLossStatistics(Statistic):

    def _forward(self, **graph_nodes):
        """
        Add to tensorboard every node which begins with FuzzyLogicLoss_

        :param graph_nodes:
        :return: Tf Summaries
        """
        summaries = []
        prefix = "FuzzyLogicLoss_"
        ...
        if len(summaries) == 0:
            error_msg = "No summaries to add, this is probably because there are no nodes in graph_nodes beginning " \
                        "with 'FuzzyLogicLoss', this statistic should not have been added, assuming this is an " \
...
class _WMCStatistic(Statistic):
    """
    Base class for all WMC statistics computed using fuzzy logic losses.
    Given the class name it will automatically check if the wmc node already exist in graph_nodes, if it does it
    is returned as a summary, otherwise the fuzzy logic loss related to the wmc to log is instantiated, and its
    ...
    def _forward(self, **graph_nodes):
        """
        Add to tensorboard the wmc from the fuzzy logic loss.
        """
        logger = self.experiment["LOGGER"]
        wmc_node_name = "FuzzyLogicLoss_" + self.constraint_name + "_wmc"
        logger.info("Doing summary for {}".format(wmc_node_name))
        ...
        if wmc_node_name in graph_nodes:
            logger.info("{} found in graph_nodes, adding it to summaries".format(wmc_node_name))
            wmc_node = graph_nodes[wmc_node_name]
            if FuzzyLogicLossStatistics in self.experiment["GENERATOR_STATISTICS"]:
                warning_msg = "{} is already present in the summaries, this summary will likely be duplicated, you " \
                              "should check your experiment file.".format(wmc_node_name)
                logger.warning(warning_msg)
        else:
            logger.info("{} not found in graph_nodes, importing sdd and creating TF AC".format(wmc_node_name))
            fuzzy_logic_loss = getattr(losses, "FuzzyLogicLoss_{}".format(self.constraint_name))(self.experiment)
            nodes, _, _ = fuzzy_logic_loss(**graph_nodes)
            wmc_node = nodes[wmc_node_name]
            del fuzzy_logic_loss
        with tf.variable_scope("generator", reuse=True):
            return tf.summary.merge([tf.summary.scalar("WMC_fuzzy_{}".format(self.constraint_name), wmc_node)])


class _VNUStatistic(Statistic):
    """
    Base class for all VNU (validity, novelty, uniqueness) statistics computed using fuzzy logic losses.
    Validity is computed using the related fuzzy logic loss to decide which samples are valid and which ones are not,
    ...
    Given the related fuzzy logic loss, an instance of this class will add the following statistics to tensorboard:
    ...
    def _forward(self, **graph_nodes):
        self.logger.info("Importing fuzzy logic loss")
        fuzzy_logic_loss = getattr(losses, "FuzzyLogicLoss_{}".format(self.constraint_name))(self.experiment)
        self.logger.info("Building graph gor VNU stats")
        summaries = self._vnu_stats(graph_nodes, fuzzy_logic_loss)
        return summaries

    def _vnu_stats(self, graph_nodes, fuzzy_logic_loss):
        """
        :param graph_nodes:
        :param fuzzy_logic_loss:
        :return:
        """
        """
        ...
        This is done because the fuzzy_logic_loss forward method expects/uses G_output_logit as its input node.
        """
        ...
        fuzzy_logic_loss.use_sigmoid = False
        nodes, _, _ = fuzzy_logic_loss(**nodes_tmp)
...
#############################################
"""
For each fuzzy logic loss class create a related WMC statistics and VNU statistics class, automatically.
"""
...
for fuzzy_loss_name in utils_common.get_module_classes(losses.__name__):
    constraint_name = "_".join(fuzzy_loss_name.split("_")[1:])
    name = "WMC_fuzzy_{}".format(constraint_name)
    if name not in globals():
        _create_wmc_class(name)

    name = "VNU_fuzzy_{}".format(constraint_name)
    if name not in globals():
